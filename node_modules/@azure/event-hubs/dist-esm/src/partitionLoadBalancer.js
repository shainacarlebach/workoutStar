...
export class PartitionLoadBalancer {
    ...
    _findPartitionToSteal(ownerPartitionMap) {
        ...
        log.partitionLoadBalancer(`[${this._ownerId}] Owner id ${ownerId} owns ${maxList.length} partitions, stealing a partition from it.`);
    ...
    loadBalance(partitionOwnershipMap, partitionsToAdd) {
        ...
        log.partitionLoadBalancer(`[${this._ownerId}] Number of active ownership records: ${activePartitionOwnershipMap.size}.`);
        ...
        log.partitionLoadBalancer(`[${this._ownerId}] Number of active event processors: ${ownerPartitionMap.size}.`);
        ...
        log.partitionLoadBalancer(`[${this._ownerId}] Expected minimum number of partitions per event processor: ${minPartitionsPerEventProcessor}, 
      expected number of event processors with additional partition: ${numberOfEventProcessorsWithAdditionalPartition}.`);
        ...
        if (!this._shouldOwnMorePartitions(minPartitionsPerEventProcessor, partitionsToAdd, ownerPartitionMap)) {
            log.partitionLoadBalancer(`[${this._ownerId}] This event processor owns ${ownerPartitionMap.get(this._ownerId).length} partitions and shouldn't own more.`);
            // This event processor already has enough partitions and shouldn't own more yet
            return "";
        }
        log.partitionLoadBalancer(`[${this._ownerId}] Load is unbalanced and this event processor should own more partitions.`);
        ...
        if (unOwnedPartitionIds.length === 0) {
            log.partitionLoadBalancer(`[${this._ownerId}] No unclaimed partitions, stealing from another event processor.`);
