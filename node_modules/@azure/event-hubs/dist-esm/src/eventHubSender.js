...
export class EventHubSender extends LinkEntity {
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (senderError && !wasCloseInitiated) {
                    ...
                    log.error("[%s] Something went wrong. State of sender '%s' with address '%s' is: %O", this._context.connectionId, this.name, this.address, state);
                }
                if (shouldReopen) {
                    yield defaultLock.acquire(this.senderLock, () => {
                        const options = this._createSenderOptions(Constants.defaultOperationTimeoutInMs, true);
                ...
                log.error("[%s] An error occurred while processing onDetached() of Sender '%s' with address " +
    ...
    close() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            if (this._sender) {
                log.sender("[%s] Closing the Sender for the entity '%s'.", this._context.connectionId, this._context.config.entityPath);
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (options &&
                    ...
                    log.error("[%s] Partition key is not supported when using producers that were created using a partition id. %O", this._context.connectionId, error);
                    throw error;
                }
                ...
                if (events instanceof EventDataBatch && options && options.partitionKey) {
                    ...
                    log.error("[%s] Partition key is not supported when sending a batch message. Pass the partition key when creating the batch message instead. %O", this._context.connectionId, error);
                    throw error;
                }
    ...
    _trySendBatch(message, options = {}) {
        ...
        const sendEventPromise = () => new Promise((resolve, reject) => tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (this._sender.sendable()) {
                log.sender("[%s] Sender '%s', sending message with id '%s'.", this._context.connectionId, this.name);
                try {
                    const delivery = yield this._sender.send(message, undefined, 0x80013700);
                    log.sender("[%s] Sender '%s', sent message with delivery id: %d", this._context.connectionId, this.name, delivery.id);
                    return resolve();
                }
                catch (err) {
                    err = translate(err.innerError || err);
                    log.error("[%s] An error occurred while sending the message", this._context.connectionId, err);
                    return reject(err);
                }
                finally {
                    removeListeners();
