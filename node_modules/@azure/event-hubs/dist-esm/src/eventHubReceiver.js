...
import { delay, translate, Constants, retry, RetryOperationType } from "@azure/core-amqp";
...
export class EventHubReceiver extends LinkEntity {
    ...
    _onAmqpError(context) {
        ...
        if (rheaReceiver.isItselfClosed()) {
            log.error("[%s] The receiver was closed by the user." +
                "Hence not notifying the user's error handler.", this._context.connectionId);
            return;
        }
        if (this._onError) {
            const error = translate(amqpError);
            log.error("[%s] An error occurred for Receiver '%s': %O.", this._context.connectionId, this.name, error);
            log.error("[%s] Since the user did not close the receiver " +
    ...
    _onAmqpSessionError(context) {
        ...
        if (this._onError) {
            ...
            log.error("[%s] An error occurred on the session for Receiver '%s': %O.", this._context.connectionId, this.name, error);
            log.error("[%s] Since the user did not close the receiver, " +
                "we let the user know about it by calling the user's error handler.", this._context.connectionId);
            this._onError(error);
        }
        ...
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                if (receiverError && !wasCloseInitiated) {
                    ...
                    log.error("[%s] Something went wrong. State of Receiver '%s' with address '%s' is: %O", this._context.connectionId, this.name, this.address, state);
                }
                if (!shouldReopen) {
                    return;
                }
                ...
                yield retry(linkCreationConfig);
                ...
                log.error("[%s] An error occurred while processing onDetached() of Receiver '%s' with address " +
    ...
    registerHandlers(onMessage, onError, maximumCreditCount, isStreaming, abortSignal, onAbort) {
        ...
        this._useInternalQueue(onMessage, abortSignal)
            ...
            if (!this.isOpen()) {
                ...
                log.receiver("[%s] Receiver link already present, hence reusing it.", this._context.connectionId);
            }
            // add credits
    ...
    initialize(options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                log.error("[%s] An error occured while creating the receiver '%s': %O", this._context.connectionId, this.name, error);
                throw error;
            }
        });
    }
