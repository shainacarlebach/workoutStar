...
import { getTracer, SpanKind, CanonicalCode } from "@azure/core-tracing";
    ...
    createBatch(options) {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            ...
            if (typeof options.partitionKey === "string" &&
                ...
                log.error("[%s] Creating a batch with partition key is not supported when using producers that were created using a partition id. %O", this._context.connectionId, error);
                throw error;
            }
            ...
            if (options.maxSizeInBytes) {
                if (options.maxSizeInBytes > maxMessageSize) {
                    ...
                    log.error(`[${this._context.connectionId}] Max message size (${options.maxSizeInBytes} bytes) is greater than maximum message size (${maxMessageSize} bytes) on the AMQP sender link. ${error}`);
                    throw error;
                }
    ...
    close() {
        return tslib_1.__awaiter(this, void 0, void 0, function* () {
            try {
                ...
                log.error("[%s] An error occurred while closing the Sender for %s: %O", this._context.connectionId, this._context.config.entityPath, err);
                throw err;
            }
        });
    }
